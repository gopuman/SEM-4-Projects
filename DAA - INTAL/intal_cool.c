...
void* intal_decrement(void* var){
	...
	n = intal->len;
	i = 0;
	if(n==1 && intal->num[0]==0){	// If the number is already zero 
...
number* karatsuba(number* var1, number* var2){
	...
	bigger = tmpSumA->len > tmpSumB->len? 2*tmpSumA->len : 2*tmpSumB->len;
...
void* intal_multiply(void* var1, void* var2){
	...
	for(i=0;i<arg1->len && i<size;i++)		
		intal1->num[i] = arg1->num[i];
	for(;i<size;i++)		//padding with zeroes
		intal1->num[i] = 0;
	intal1->len = len;
	for(i=0;i<arg2->len && i < size;i++)
